...
export async function parseLogo(url: string): Promise<LogoEntry | null> {
  console.log(`\n[INFO] Processing: ${url}`);
...
export async function parseLogos(urls: string[]): Promise<{ domain: string; status: 'success' | 'error'; entry?: LogoEntry; error?: string }[]> {
  const limit = pLimit(config.concurrency);
  const tasks = urls.map(url =>
    limit(async () => {
      try {
        const entry = await parseLogo(url);
        if (entry) {
          return { domain: entry.domain, status: 'success' as const, entry };
        }
        return { domain: getDomain(url), status: 'error' as const, error: 'No logo found' };
      } catch (error) {
        return { domain: getDomain(url), status: 'error' as const, error: (error as Error).message };
      }
    })
  );
